/**
 * 客户端自动更新器:基于 GitHub Releases(公开仓库,免鉴权)。
 *
 * 流程:
 *   checkUpdate(repo)   → 查 releases/latest,semver 对比,决定是否有新版
...
async function checkUpdate(repo) {
  ...
  try {
    resp = await fetch(`${API_BASE}/repos/${repo}/releases/latest`, {
      headers: { Accept: 'application/vnd.github+json', 'User-Agent': 'sku-manager-updater' },
      signal: ctrl.signal,
    });
  } catch (e) {
  ...
  if (resp.status === 404) return { ok: false, error: '仓库或 Release 不存在,请检查 update_repo' };
  if (!resp.ok) {
    if (resp.status === 403) return { ok: false, error: 'GitHub 限流(403),稍后再试' };
...
async function stageUpdate(info, onProgress) {
  ...
  await download(info.shaUrl, shaPath);
...
function download(url, outPath, onProgress) {
  return new Promise((resolve, reject) => {
    ...
    fetch(url, {
      signal: ctrl.signal,
      redirect: 'follow',
      headers: { 'User-Agent': 'sku-manager-updater' },
    })
      .then(async (resp) => {
