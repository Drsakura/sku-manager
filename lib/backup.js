...
async function createBackup() {
  ...
  try {
    // 密钥不能跟着备份文件走 —— 备份是拿来传给别的设备/别人的,
    // 里面带着 GitHub token 或云端 API Key 等于把凭据一起送出去。
    const { SECRET_KEYS } = require('./settings');
    const del = out.prepare('DELETE FROM settings WHERE key = ?');
    for (const k of SECRET_KEYS) del.run(k);
...
async function restoreBackup(filePath) {
  ...
  // 本机密钥要在还原后保留:备份里没有(导出时已剔除),
  // 若不留一份,还原会把本机已配好的 token / API Key 一起清掉。
  const { SECRET_KEYS } = require('./settings');
  const keptSecrets = db
    .prepare(
  ...
  try {
    db.transaction(() => {
      ...
      // 把本机密钥写回去
      const restoreSecret = db.prepare(
        'INSERT INTO settings (key, value) VALUES (?, ?) ON CONFLICT(key) DO UPDATE SET value = excluded.value'
      );
      for (const s of keptSecrets) restoreSecret.run(s.key, s.value);
